...
def aesKeygenerator():
    ...
    aes_key = reduce(lambda a, b: a + b, [choice(ascii_letters.replace(" "," !@#$^&*-=;.")) for i in range(16)])
    print(f'AES secret key: {aes_key}') #Debug print
    return aes_key

def initializeVectorgenerator():
    initialization_vector = reduce(lambda a, b: a + b, [choice(ascii_letters.replace(" "," !@#$^&*-=;.")) for i in range(16)])
